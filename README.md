...
从 GitHub Releases 下载对应平台的二进制并直接运行（自动拉取最新版）。默认监听 `0.0.0.0:4141`（全部网卡），默认管理员密码 `admin123`（首次登录强制修改）。
...
直接运行二进制则默认监听全部网卡 `http://0.0.0.0:4141`；可用环境变量 `M365_LISTEN=127.0.0.1:4141` 限制为仅本机访问。
...
浏览器打开控制台（默认 `http://127.0.0.1:4141`；如改过监听地址请替换成对应主机）：
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `M365_LISTEN` | `0.0.0.0:4141` | 监听地址，默认全部网卡；设 `127.0.0.1:4141` 仅本机可访问 |
...
- **默认监听全部网卡**：直接运行二进制默认 `M365_LISTEN=0.0.0.0:4141`，无需额外配置即可被反代、容器端口映射或隧道访问；如需限制为仅本机，设为 `M365_LISTEN=127.0.0.1:4141`。对外提供服务务必通过 TLS 终泄反向代理（Nginx / Caddy），并为 SSE 与 WebSocket 开启长连接与 `proxy_buffering off`。
